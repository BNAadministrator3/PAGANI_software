...
void sparsevv_plus(int ns, int *c,float *x, real_t *y, float alpha)
{
	for (int i = 0; i < ns; i++)
		y[c[i]] += alpha * x[i];
}

real_t element_ij(real_t **kmn, int **kmn_c, int i, int j, int nc)
{
	real_t x=0;
	for (int k = 0; k < nc; k++)
	{
...
void expand(int n, int *c, real_t *x, real_t *y)  //sparse x convert to dense y
{
	for (int i = 0; i < n; i++)
		y[c[i]] = x[i]; 
}

void compress(int **kmn_Cj, real_t **kmnj,int *kmnN, real_t *kmn_j, int n)
{
	int i = 0;
	int idx = 0;
	int *kmn_j_idx = new int [n];
	while (i < n)
	{
		if (!(equal<real_t>(kmn_j[i],0.0f)))
		{
			kmn_j_idx[idx] = i;
			kmn_j[idx] = kmn_j[i];
			idx++;
	...
	*kmn_Cj = new int [idx];
	if (idx>0) 
	{
		memcpy(*kmnj, kmn_j, sizeof(real_t)*idx);
		memcpy(*kmn_Cj,kmn_j_idx,sizeof(int)*idx);
	}
	delete [] kmn_j_idx;
}
...
double Louvain_modularity_sub1(  real_t **W, int *R, int *C, float *V, long long *n1, int *Ci, long long N, double s)
{
	...
	int *kmn_N = new int [n0];
	int **kmn_C = new int* [n0];
	
	real_t **kmn = new real_t* [n0];
	real_t *k = new real_t [n0];
	memset(k,0,sizeof(real_t)*n0);
	...
	{
		kmn_N[i] = R[i+1]-R[i];
		kmn_C[i] = new int [kmn_N[i]];
		kmn[i] = new real_t [kmn_N[i]];	
		idx = 0;
		for (j = R[i]; j < R[i+1]; j++)
		{
			k[i] += V[j];
	...
	*/		
	...
	while (flag)
	{
		...
		{   i=a[idx];
			...
			//cout<<"i = "<<i<<endl;
			double tmp1 = 0.0 - element_ij(kmn,kmn_C, M[i], i, kmn_N[M[i]]);
			...
			for (j = 0; j < n0; j++)
				dQ[j]=(element_ij(kmn,kmn_C, j, i, kmn_N[j])+tmp1) - k[i]*(km[j]+tmp2)*s;
			...
			if (max_dQ > ep)
			{				
				real_t *kmn_j = new real_t [n0];
				memset(kmn_j,0,sizeof(real_t)*n0);
				expand(kmn_N[j],kmn_C[j],kmn[j],kmn_j);
				sparsevv_plus((R[i+1]-R[i]),C+R[i],V+R[i], kmn_j, 1.0f);
				compress(&kmn_C[j],&kmn[j],kmn_N+j, kmn_j,n0);

				memset(kmn_j,0,sizeof(real_t)*n0);
				expand(kmn_N[M[i]],kmn_C[M[i]],kmn[M[i]],kmn_j);
				sparsevv_plus((R[i+1]-R[i]),C+R[i],V+R[i], kmn_j, -1.0f);
				compress(&kmn_C[M[i]],&kmn[M[i]],kmn_N+M[i], kmn_j,n0);
				//int z=0;
				//for (z=R[i]; z<R[i+1]; z++)
				//{
				//	*(knm+C[z]*n0+j) += V[z];
...
double Louvain_modularity( int *Ci, int *R, int *C, float *V, long long N, int *Num_module)
{   
	double s = 0;
	double q = 0, q_new = 0;
	long long n0 = N;
...
int main(int argc, char * argv[])
{
	...
	for (i = 0; i < FileNumber; i++)
	{
		...
		//real_t *W = new real_t[1];
		//memset(W, 0, sizeof(real_t));
		...
		*/		
		...
		Setup(0);
		Start(0);
		double q = Louvain_modularity(Ci, R_new, C_new, V, N_C, &Nm);
		...
		*/
		...
		for (int l = 0; l < Maslov_num; l++)
		{
			...
			q = Louvain_modularity(Ci,  R_dst, C_dst, V_dst, N_C,&Nm);
			fout<<"Final Modularity Q = "<<q<<endl;
			fout<<"\n*****************************************************\n\n";

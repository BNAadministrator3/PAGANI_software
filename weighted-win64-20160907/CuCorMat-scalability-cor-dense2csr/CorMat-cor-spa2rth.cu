...
const int thread_num = 512; //maybe redefinition
...
*/
real__t CorMat_spa2rth(string OutCor, real__t * BOLD_t,const int &N, const int &L, const int & Batch_size,real__t *s_thresh, real__t* result, const int & num_spa)
{
	...
	{
		zeroAmount[i] = N * s_thresh[i] * (N-1)  / 100.0 ;
		zeroAmount[i] -= (N * N);
		zeroAmount[i] *= -1;
	}

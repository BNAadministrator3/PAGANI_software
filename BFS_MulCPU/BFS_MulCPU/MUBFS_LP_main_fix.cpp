...
double APSP_BFS(int * row, int * col, const int numVertices);
float *Li_result;
#define LONG_FLAG
...
int main(int argc, char** argv) 
{
	...
	for (int i = 0; i < FileNumber; i++)
	{
		...
		//fCSR.read((char*)row, sizeof(long long)*(numVertices + 1));
		#ifdef LONG_FLAG
		long long *rowx = new long long [numVertices+1];
		fCSR.read((char*)rowx, sizeof(long long)*(numVertices + 1));
		for (int j = 0; j <= numVertices; j++)
			row[j] = (int) rowx[j];
		delete []rowx;
		long long E_temple = 0;
		fCSR.read((char *)&E_temple, sizeof(long long));
		numEdges = E_temple;
		#else
